...
export function App() {
  ...
  const [profile, setProfile] = useState<CoupleProfile>(() => {
    ...
    if (saved) {
      ...
      if (parsed.husbandName === 'Minh Tuấn' || parsed.husbandName === 'Chồng' || parsed.husbandName?.includes('Phan Thanh Tuấn')) {
        return initialProfile;
      }
      if (parsed.husbandAvatar?.includes('unsplash.com')) {
        parsed.husbandAvatar = initialProfile.husbandAvatar;
      }
      if (parsed.wifeAvatar?.includes('unsplash.com')) {
        parsed.wifeAvatar = initialProfile.wifeAvatar;
      }
      return parsed;
    }
    return initialProfile;
  ...
  // Auto-play music on mount & fallback to first user interaction
  useEffect(() => {
    let unmounted = false;
    ...
    const startAudio = () => {
      if (audioRef.current && !unmounted) {
        audioRef.current.play().then(() => {
          ...
          // If browser blocked unmuted autoplay, start on first click/touch/scroll
  ...
  return (
    <div className="min-h-screen bg-rose-50/30 text-slate-800 pb-20">
      ...
      {/* Music Player */}
      <MusicPlayer
        musicUrl={profile.backgroundMusicUrl || 'https://www.youtube.com/watch?v=wJvX33mPllg'}
